...
const navigation = [
  ...
  { name: 'Performers', href: '/performers', icon: Mic2, section: 'CONTRIBUTORS' },
  { name: 'Producers & Engineers', href: '/producers', icon: Headphones, section: 'CONTRIBUTORS' },
  { name: 'Writers', href: '/writers', icon: PenTool, section: 'CONTRIBUTORS' },
  { name: 'Publishers', href: '/publishers', icon: Building2, section: 'CONTRIBUTORS' },
  { name: 'Labels', href: '/labels', icon: Disc3, section: 'CONTRIBUTORS' },
  { name: 'Royalties', href: '/royalties', icon: DollarSign, section: 'FINANCIAL' },
...
export function Sidebar({ isMobileMenuOpen = false, onCloseMobileMenu }: SidebarProps) {
  ...
  const sections = [
    { name: 'ASSETS', items: navigation.filter(item => item.section === 'ASSETS') },
    { name: 'CONTRIBUTORS', items: navigation.filter(item => item.section === 'CONTRIBUTORS') },
    { name: 'FINANCIAL', items: navigation.filter(item => item.section === 'FINANCIAL') },
    { name: 'SETTINGS', items: navigation.filter(item => item.section === 'SETTINGS') },
  ];

  return (
    <>
      {/* Desktop Sidebar */}
      <div className={`hidden lg:flex h-screen flex-col bg-[#0a0a0a] border-r border-gray-800 transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}>
        {/* Logo */}
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          {sections.map((section) => (
            <div key={section.name} className="mb-6">
              {!isCollapsed && (
                <div className="mb-2 px-3 text-xs text-gray-500">{section.name}</div>
              )}
                ...
                {section.items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {!isCollapsed && <span className="text-[15px] font-semibold">{item.name}</span>}
                    </Link>
                  );
                })}
              </div>
